...
async function insertTrainingPrograms() {
  try {
    console.log('🌱 Inserting training programs...');
    
    // Insert each program individually
    for (const programData of trainingProgramsData) {
      try {
        ...
        console.log(`❌ Failed to create ${programData.title}:`, error.message);
        console.log('Error details:', error);
      }
    }
    
    console.log('🎉 Training programs insertion completed!');
    
    // Test the API
    console.log('\n📊 Testing API...');
    const response = await fetch('http://localhost:5000/api/training-programs');
    const data = await response.json();
    console.log(`Found ${data.count} training programs in database`);
    
  } catch (error) {
    console.error('❌ Error inserting training programs:', error.message);
  } finally {
